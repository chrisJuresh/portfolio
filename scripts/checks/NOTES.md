...
| `front-screen`   | the Front Screen's rhyme, its one-screen budget, the Cut Title's cut or its accessible name, the crossing's span, the switch's ARIA, or the type's place in the Effect Stack breaks — at rest, and again with every track of the reveal held in its own middle, where a box that groups outside the lifted blocks would seal the type under the stack |
...
**Its fifteenth group is the clearest case in this file of a Check standing in
for a picture that cannot tell two things apart** (#201). The grid behind the
composition is four horizontals and three verticals, and none of them is a
number: the horizontals are the four Points' own rules continued and the
verticals are the three standing blocks' left edges, taken through
`grid-template-columns: subgrid` so the twelve tracks are inherited rather than
restated. A grid at six typed fractions and this one are the SAME SCREENSHOT at
the window the fractions were typed at — the mockup's own 0.505 and 0.735 were
Points 03 and 04 to within a per cent — and different pictures at every other. So
there is nothing here for a person to notice failing, which is the definition.

**It reads three windows and the third is not padding either**: the grid's two
halves answer the regime differently while neither is told which regime it is in,
so a run that never leaves the band asserts only the half that is the same in
both. And **it does not assert paint order**, though "behind everything" is what
the ticket asked for: what it asserts is that the grid takes no hit, because that
is the half a reader is deprived of — three vertical strips of the composition
they can no longer point at, which on this Section takes the Drop with it. Which
of two hairlines is on top of the other is a look. Breaking it four ways at once
— `subgrid` restated as `repeat(12, …)`, the horizontal's `top` zeroed, the
collapse's `display: none` removed, `aria-hidden` removed — failed at all three
windows; a fifth, making the box hit-testable, took three more assertions and the
Drop's whole group with it.
...
**`eater-map` stands the page on that Section's OWN resting place, found from its
snap position, and not on the last port.** The two were the same number for as long
as the Eater Map was the last Section, and stopped being when the Catalogue landed
after it (ADR 0008): three groups stood the page on a Section with no Lift in it
while every message they wrote named this one. `restingPlaces` in the Check is that
lookup, and a page where no port agrees with the Section is reported as such rather
than as a Lift that never ran. **A Check that reaches for "the last" of anything is
asserting the document's shape**, which is the page's business and not the Check's —
the `turn` Check's own note about counting ports says the same.
...
**The fifth is live only while an entry is unbuilt.** The Catalogue took the Rail's
third entry (ADR 0008), so every entry is a link, and the vacuous case — nothing
unbuilt, so nothing to read — is a note rather than the failure it was written as:
a blocking Check may not fail the page's legitimate shape. The assertion comes back
the day a Showcase is promised and not yet built, with nothing changed here.
...
**The reveal is a track per block now, and the assertion followed it.** The
Section-wide fade is what a reader who asked for less motion gets; everyone else
gets a mask, a filter or a translate on each block, on its pseudo-elements, and on
the Cut Title. So the restart clears `animation-name` across the whole Section,
the hold puts EACH track at its own middle — staggered tracks held at one clock
time are mostly at an end, where nothing groups — and the question asked of each
grouping box is which z it is painted at, which is the old question asked of every
box instead of one. `settle()` waits for finite document-timeline animations under
every Section for the same reason `revealed()` does: on a warm cache the network
goes quiet while lines of type are still rising, and `moments` saw a listing entry
move a quarter of a pixel through a held Timeline because of it.
