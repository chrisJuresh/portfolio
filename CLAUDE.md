...
place per Section and nothing between them: one wheel PUSH carries the reader
from one to the next — a push, not an event and not a whole gesture, because a
trackpad delivers one flick as a rise and then a decaying tail still arriving a
second later, and the two failures are chaining through every port (#205) and
refusing the reader's next flick until the first one's momentum has died (#210) —
and on the FIRST notch the Front Screen's cut PROJECTS
