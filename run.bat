...
rem this one is the site - it builds the tree and serves the build - watch.bat is
rem that same build kept up to date with the branch, rebuilding and reloading the
rem page as commits land - and `pnpm dev` is the one to WORK in, reloading as you
rem edit. `pnpm editor` - or editor.bat, the same thing double-clicked - is how
